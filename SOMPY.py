...
class SOM(object):
    ...
    # Set SOM topology
    def set_topology(self, mapsize=None, mapshape='planar', lattice='rect',
                     mask=None, compname=None):
        """
    ...
    def calc_map_dist(self):
        """Calculating the grid distance, which will be called during the
        training steps.
    ...
    def set_algorithm(self, initmethod='pca', algtype='batch',
                      neighborhoodmethod='gaussian', alfatype='inv',
                      alfaini=0.5, alfafinal=0.005):
        """
        initmethod = ['random', 'pca']
        algos = ['seq', 'batch']
        all_neigh = ['gaussian', 'manhatan', 'bubble', 'cut_gaussian',
                     'epanechicov' ]
    ...
    def view_map(self, what='codebook', which_dim='all', pack='Yes',
                 text_size=2.8, save='No', save_dir='empty', grid='No',
                 text='Yes'):
        """Visualize map."""
    ...
    def init_map(self):
        ...
        if getattr(self, 'initmethod') == 'random':
            ...
            mn = np.tile(np.min(getattr(self, 'data'), axis=0),
            ...
            print 'Set a correct one in SOM. ' + \
                'Current SOM.initmethod: {}'.format(getattr(self, 'initmethod'))
    ...
    def train(self, trainlen=None, n_job=1, shared_memory='no', verbose='on'):
        ...
        # Initialization
        if verbose == 'on':
            print '\nInitialization method = {}, initializing...\n'.format(
        ...
        if verbose == 'on':
            ts = round(time() - t0, 3)
            print "\nTotal time elapsed: {} seconds.".format(ts)
            print "final quantization error: {}".format(err)

    def project_data(self, data):
        """Project a data set to a trained SOM and find the index of BMU.

        It is based on nearest neighborhood search module of scikit.learn, but
        it is not that fast.
        """
    ...
    def predict_by(self, data, Target, K=5, wt='distance'):
        ...
        Here it is assumed that Target is the last column in the codebook and
        data has dim-1 columns.
        """
        codebook = getattr(self, 'codebook')
        data_raw = getattr(self, 'data_raw')
    ...
    def predict(self, X_test, K=5, wt='distance'):
        ...
        Similar to scikits.learn we assume that we have X_tr, Y_tr and X_test.
        Here it is assumed that Target is the last column in the codebook and
        data has dim-1 columns
        """
        codebook = getattr(self, 'codebook')
    ...
    def hit_map(self, data=None):
        ...
        a = plt.hist2d(coord[:, 1], coord[:, 0], bins=(msz[1], msz[0]),
                       alpha=0.0, norm= LogNorm(), cmap=cm.jet)
        # clbar  = plt.colorbar()
        x = np.arange(0.5, msz[1] + 0.5, 1)
        y = np.arange(0.5, msz[0] + 0.5, 1)
        X, Y = np.meshgrid(x, y)
        area = a[0].T * 12
        plt.scatter(X, Y, s=area, alpha=0.2, c='b', marker='o', cmap='jet',
                    linewidths=3, edgecolor='r')
        plt.scatter(X, Y, s=area, alpha=0.9, c='None', marker='o', cmap='jet',
                    linewidths=3, edgecolor='r')
        plt.xlim(0, msz[1])
        plt.ylim(0, msz[0])
        ...
        if data is not None:
            ...
            plt.scatter(X, Y, s=area, alpha=0.2, c='b', marker='o', cmap='jet',
                        linewidths=3, edgecolor='r')
            plt.scatter(X, Y, s=area, alpha=0.9, c='None', marker='o',
    ...
    def hit_map_cluster_number(self, data=None):
        ...
        if data is None:
            ...
            for i, txt in enumerate(codebook):
                ax.annotate(txt, (cents[i, 1], cents[i, 0]), size=10,
                            va='center')
        ...
        # plt.pcolor(codebook.reshape(msz[0], msz[1])[::-1], alpha=0.5,
        #     cmap='jet')
        plt.show()
        return codebook
    ...
    def predict_Probability(self, data, Target, K=5):
        ...
        # Predicted_values = clf.predict(data)
        # Predicted_values = denormalize_by(data_raw[:, Target],
        #                                   Predicted_values)
    ...
    def node_Activation(self, data, wt='distance', Target=None):
        ...
        if Target is None:
            codebook = getattr(self, 'codebook')
            data_raw = getattr(self, 'data_raw')
            clf = neighbors.KNeighborsClassifier(n_neighbors=getattr(self,
                                                                     'nnodes'))
            labels = np.arange(0, codebook.shape[0])
            clf.fit(codebook, labels)
            # The codebook values are all normalized.
    ...
    def update_codebook_voronoi(self, training_data, bmu, H, radius):
        ...
        smaller matrix. Super fast comparing to classic batch training
        algorithm.
        ...
        # H has nnodes * nnodes and S has nnodes * dim ->
    ...
    def grid_dist(self, bmu_ind):
        """
        som and bmu_ind
        depending on the lattice "hexa" or "rect" we have different grid
        distance functions.
        bmu_ind is a number between 0 and number of nodes-1. depending on the
        map size
        bmu_coord will be calculated and then distance matrix in the map will be
        returned
        """
        try:
    ...
    def rect_dist(self, bmu):
        """The way we consider the list of nodes in a planar grid is that node0
        is on top left corner, nodemapsz[1]-1 is top right corner and then it
    ...
    def view_2d(self, text_size, which_dim='all', what='codebook'):
        ...
        if what == 'codebook':
            ...
            norm = matplotlib.colors.normalize(vmin=np.mean(codebook.flatten()) - 1
                                            * np.std(codebook.flatten()),
                                            vmax=np.mean(codebook.flatten()) + 1
                                            * np.std(codebook.flatten()),
                                            clip=True)
            while axisNum < dim:
                axisNum += 1
    ...
    def view_2d_Pack(self, text_size, which_dim='all', what='codebook',
                     save='No', grid='Yes', save_dir='empty', text='Yes'):
        msz0, msz1 = getattr(self, 'mapsize')
        if what == 'codebook':
            if hasattr(self, 'codebook'):
                codebook = getattr(self, 'codebook')
            ...
            h = 0.2
            w = 0.001
            fig = plt.figure(figsize=(no_col_in_plot * 1.5 * (1 + w),
                                      no_row_in_plot * 1.5 * (1 + h)))
            # print no_row_in_plot, no_col_in_plot
            norm = matplotlib.colors.Normalize(vmin=np.median(codebook.flatten()) -
                                            1.5 * np.std(codebook.flatten()),
                                            vmax=np.median(codebook.flatten()) +
                                            1.5 * np.std(codebook.flatten()),
                                            clip=False)

            DD = pd.Series(data=codebook.flatten()).describe(
                percentiles=[.03, .05, .1, .25, .3, .4, .5, .6, .7, .8, .9, .95,
                             .97])
            norm = matplotlib.colors.Normalize(vmin=DD.ix['3%'],
                                               vmax=DD.ix['97%'], clip=False)
        ...
        if save == 'Yes':
                if save_dir != 'empty':
                    ...
                    fig.savefig(add, bbox_inches='tight', transparent=False,
                                dpi=200)
                plt.close(fig)
